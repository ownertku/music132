    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                if not file_path:
                    raise AssistantErr("Failed to download media. Please update yt-dlp on your VPS using `pip3 install -U yt-dlp`.")
        ...
        try:
            ...
            print(f"[DEBUG] download result: file_path={file_path}, direct={direct}")
        except Exception as e:
            print(f"[DEBUG] download raised exception: {e}")
            raise AssistantErr(_["play_14"])
        
        if not file_path:
            print(f"[DEBUG] file_path is None/empty, raising error")
            raise AssistantErr("Failed to download media. Please update yt-dlp on your VPS using `pip3 install -U yt-dlp`.")
